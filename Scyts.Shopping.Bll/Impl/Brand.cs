...
{
    class Brand : Common<Model.Brand>, IBrand
    {
        ...
        public Result<int> GroupDelete(List<int> ids)
        {
            return ExecuteDelegate.ExeTran<int>(delegate () {
                using (MyDbContext db = new S_DbContext())
                {
                    ...
                    StringBuilder sb = new StringBuilder("delete from tb_g_brand where Brand_Id in ");
                    ...
                    sb.Append(delIds);
                    sb.Append(";");
                    sb.Append("delete from tb_G_Category_Brand where Brand_Id in");
                    sb.Append(delIds);
